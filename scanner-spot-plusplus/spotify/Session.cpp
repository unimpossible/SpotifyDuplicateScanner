...
{
	//init and send to updated method
	sp_session_callbacks user_callbacks = {0}; 
	return Session::Initialise(config, user_callbacks);
}

sp_error Session::Initialise(const Config &config, 
							 sp_session_callbacks user_callbacks) {
